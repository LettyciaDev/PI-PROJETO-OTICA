...
function ModalConfirmacao({
  ...
  useEffect(() => { if (aberto) btnRef.current?.focus(); }, [aberto]);
  ...
  return (
    ...
    >
      <div className={styles.modalSheet}>
        ...
        <p className={styles.modalTitulo}>Tudo certo? 🕶️</p>
        <p className={styles.modalSub}>Confirme seu pedido antes de ir ao carrinho</p>

        <div className={styles.modalRow}>
          <span className={styles.modalLbl}>Produto</span>
          <span className={`${styles.modalVal} ${styles.modalValBold}`}>{capitalize(nomeProduto)}</span>
        </div>

        <div className={styles.modalDivider} />

        <div className={styles.modalRow}>
          <span className={styles.modalLbl}>Cor</span>
          <span className={styles.modalVal}>
            {corHex && (
              <span
                className={styles.modalBolinha}
                style={{ background: corHex }}
              />
            )}
            {capitalize(corSelecionada)}
          </span>
        </div>

        <div className={styles.modalRow}>
          <span className={styles.modalLbl}>Lentes</span>
          <span className={styles.modalVal}>
            {lentesSelecionadas.length === 0
              ? <span className={styles.modalTagVazia}>Padrão (sem extras)</span>
              : (
                <span className={styles.modalTagsWrap}>
                  {lentesSelecionadas.map((l) => (
                    <span key={l} className={styles.modalTag}>{l}</span>
                  ))}
                </span>
              )
            }
        ...
        <div className={styles.modalRow}>
          <span className={styles.modalLbl}>Quantidade</span>
          <span className={styles.modalVal}>{quantidade} un.</span>
        </div>

        <div className={styles.modalDivider} />

        <div className={styles.modalRow}>
          <span className={styles.modalLbl}>Total</span>
          <span className={styles.modalPreco}>R$ {precoTotal}</span>
        </div>

        <div className={styles.modalBtnRow}>
          <button className={styles.modalBtnVoltar} onClick={onFechar}>
            Voltar
          </button>
          <button
            ref={btnRef}
  ...
  return (
    ...
    >
      <img
        ...
        style={zoom.ativo ? { transform: `scale(${ZOOM_FACTOR})`, transformOrigin: `${zoom.x}% ${zoom.y}%` } : undefined}
        draggable={false}
      />
      {zoom.ativo && (
        <div className={styles.zoomMira} style={{ left: `${zoom.x}%`, top: `${zoom.y}%` }} />
      )}
    </div>
  );
}
...
function BotaoCompartilhar({ nomeProduto }) {
  ...
  return (
    <div className={styles.compartilharWrapper}>
      <button className={styles.btnCompartilhar} onClick={toggleMenu} aria-label="Compartilhar produto" title="Compartilhar">
      ...
      {(estado === "menu" || estado === "copiado") && (
        <div className={styles.compartilharDropdown}>
          ...
          <button className={styles.compartilharOpcao} onClick={copiarLink}>
            {estado === "copiado" ? (
              <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>Link copiado!</>
            ) : (
              <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>Copiar link</>
            )}
          </button>
        </div>
      )}
...
export default function ProdutoCliente({ oculos }) {
  ...
  const [lentes, setLentes] = useState({
    Antirreflexo: false, Antirrisco: false, "Blue Control": false,
    Fotossensível: false, "Proteção UV": false, "Hard Coat": false,
  });
  ...
  const precoFinal = precoUnitario.toFixed(2).replace(".", ",");
  ...
  return (
    <>
      <ModalConfirmacao
        ...
        onConfirmar={confirmarEIrAoCarrinho}
        nomeProduto={oculos.nome}
        corSelecionada={varianteSelecionada?.cor ?? ""}
        corHex={bgCorAtual?.solid ?? null}
        ...
        <main className={styles.pagina}>
                ...
                <div className={styles.galeriaThumbs}>
                  {todasImagens.map((img, idx) => (
                    <button
                      ...
                      className={[styles.thumbBtn, idx === indexFotoAtiva ? styles.thumbBtnAtivo : ""].join(" ")}
                      style={idx === indexFotoAtiva && bgCorAtual ? { borderColor: bgCorAtual.solid } : undefined}
                    >
            ...
            <div className={styles.colunaDetalhes}>
              ...
              {variantes.length > 0 && (
                <div className={styles.secaoCor}>
                  <p className={styles.labelSecao}>
                    Cor: <strong>{capitalize(varianteSelecionada?.cor ?? "")}</strong>
                    {!temEstoque && <span className={styles.semEstoque}> — Sem estoque</span>}
                    ...
                    {variantes.map((v) => {
                      ...
                      return (
                        <button
                          ...
                          className={[styles.botaoCor, ativo ? styles.botaoCorAtivo : "", semEstoque ? styles.botaoCorSemEstoque : ""].join(" ")}
                          style={
                            bgCor
                              ? ativo
                                ? { borderColor: bgCor.solid, background: `rgba(${hexToRgb(bgCor.solid)}, 0.1)`, color: bgCor.solid }
                                : semEstoque
                        ...
                        >
                          {bgCor && <span className={styles.bolinhaCor} style={{ background: bgCor.bg ?? bgCor.solid }} aria-hidden="true" />}
                          {capitalize(v.cor)}
                          {semEstoque && <span className={styles.riscoBotaoCor} aria-hidden="true" />}
                        </button>
                      );
                    })}
                  </div>
                ...
                <div className={styles.gridLentes}>
                  {Object.keys(lentes).map((nome) => (
                    <label key={nome} className={styles.checkboxItem}>
                      <input type="checkbox" className={styles.checkbox} checked={lentes[nome]} onChange={() => toggleLente(nome)} />
                      <span>{nome}</span>
                    </label>
                  ))}
                </div>
              ...
              <div className={styles.garantia}>
                ...
                <div>
                  ...
                  <p className={styles.garantiaSubtitulo}>Valorizamos a qualidade em cada detalhe para garantir a sua satisfação.</p>
                </div>
              </div>
              ...
              <div className={styles.compraRow}>
                <div
                  ...
                  style={bgCorAtual && temEstoque ? { borderColor: bgCorAtual.solid } : undefined}
                >
                ...
                <button
                  ...
                  onClick={temEstoque ? () => setModalAberto(true) : undefined}
                  style={
          ...
          <section className={styles.especificacoes}>
            <div className={styles.colEspec}>
              ...
              <div className={styles.EspecLinha}><span className={styles.EspecLabel}>Formato da armação</span><span>{capitalize(oculos.formato)}</span></div>
              <div className={styles.EspecLinha}><span className={styles.EspecLabel}>Tipo de material</span><span>{capitalize(oculos.material)}</span></div>
            </div>
            <div className={styles.colEspec}>
              <p className={styles.EspecTitulo}>TAMANHO DO PRODUTO</p>
              <div className={styles.EspecLinha}><span className={styles.EspecLabel}>Comprimento do aro</span><span>{oculos.medida_aro} mm</span></div>
              <div className={styles.EspecLinha}><span className={styles.EspecLabel}>Comprimento da ponte</span><span>{oculos.medida_ponte} mm</span></div>
              <div className={styles.EspecLinha}><span className={styles.EspecLabel}>Comprimento da haste</span><span>{oculos.medida_haste} mm</span></div>
            </div>
          </section>
            ...
            <div className={styles.gridAvaliacoes}>
              {avaliacoes.map((av) => (
                <div key={av.nome} className={styles.card}>
                  <div className={styles.cardImagem}><span className={styles.cardImagemTexto}>IMAGEM<br />ÓCULOS</span></div>
